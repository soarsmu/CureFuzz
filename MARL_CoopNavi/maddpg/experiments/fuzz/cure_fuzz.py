...
class RND(nn.Module):
    def __init__(self, input_size=24, hidden_size=128, output_size=128):
...
class Cure:
    def __init__(self, input_size=24, hidden_size=64, output_size=64,learning_rate=1e-4):
        self.corpus = []
        self.final_state = []
        self.rewards = []
        self.result = []
    ...
    def get_pose(self):
        ...
        for i in range(len(self.corpus)):
            prob = (math.exp(-self.rewards[i]*0.01)+self.entropy[i]+math.exp(-self.intrinsic_reward[i]))
