...
def run(model_path, port, suite, big_cam, seed, autopilot, resume, max_run=10, show=False, replay=False):
...
if __name__ == '__main__':
    ...
    if args.replay:
        run(Path(args.model_path), args.port, args.suite, args.big_cam, args.seed, args.autopilot, args.resume, max_run=args.max_run, show=args.show, replay=args.replay)
    else:
        f = open('./results'+result_folder+'/artifact.txt', 'w', buffering=1)
        sys.stdout = f
        run(Path(args.model_path), args.port, args.suite, args.big_cam, args.seed, args.autopilot, args.resume, max_run=args.max_run, show=args.show, replay=args.replay)
        f.close()