...
class Cure:
    def __init__(self, input_size=17, hidden_size=64, output_size=16,learning_rate=1e-7):
        self.corpus = []
        self.final_state = []
        self.rewards = []
        self.result = []
    ...
    def get_pose(self):
        ...
        for i in range(len(self.corpus)):
            prob = (math.exp(-self.rewards[i]*0.01)+math.exp(-self.intrinsic_reward[i]*0.1)+self.entropy[i])
    ...
    def train_rnd(self, states, intrinsic_reward_scale=3e-4, l2_reg_coeff=1e-6):
        state_tensor = torch.FloatTensor(states).to(self.device)
        target_out, predictor_out = self.rnd(state_tensor)
